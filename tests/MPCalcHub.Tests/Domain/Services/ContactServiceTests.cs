...
using Fiap.Hackatoon.Product.Consumer.Api.Controllers;
...
namespace Fiap.Hackatoon.Product.Consumer.Tests.Integration
{
    public class ContactControllerIntegrationTests : IAsyncLifetime
    {
        ...
        public async Task ShouldRemoveContact()
        {
            // Arrange
            await ShouldInsertContact();
            var contact = _contactService.GetAll().FirstOrDefault();

            // Act
            var deleteResult = await _controller.Delete(contact.Id);

            // Assert
            Assert.IsType<AcceptedResult>(deleteResult);
        }
